...
Remember to tailor your response to the individual's specific situation and any health data provided.`;
  ...
  public async getAyurvedaAdvice(
    ...
    try {
      ...
      console.log('Sending request to OpenAI with message:', userPromptWithData);
      ...
      if (this.messageHistory.length > 10) {
        this.messageHistory = [
          ...
          ...this.messageHistory.slice(-4) // Keep last 4 messages
        ];
      }
      ...
      if (error.response?.status === 401) {
        console.error('Authentication error. API Key:', this.maskApiKey(import.meta.env.VITE_OPENAI_API_KEY));
