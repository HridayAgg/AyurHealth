...
// Simplified GAPI types that focus on the essential interfaces we need
interface IGapi {
  load: (apiName: string, callback: () => void) => void;
  auth2: {
    init: (params: { client_id: string; scope: string }) => Promise<void>;
    getAuthInstance: () => {
      signIn: (options?: { scope: string }) => Promise<{ getAuthResponse: () => { access_token: string; expires_at: number } }>;
